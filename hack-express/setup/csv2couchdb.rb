...
server = Couch::Server.new("127.0.0.1", "5984", "hack-admin", "secret")
...
begin
    ...
    array_of_hashes.each do |row|
      ...
      row['users'] = [] # User JSON Array
      row['type'] = 'token' # All items being imported are currently tokens
      row['points'] = Integer(row['points']) # Points need to be ints in order to sum correctly, and not concatenate!
